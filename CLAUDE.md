...
Still missing sitewide: analytics. Real photography is done — every `.ph` box
across the site has been replaced with an actual photo or video — and the
real logo is in as of 2026-08-21 (see below).

### Logo — two real variants, don't confuse them

The delivered package (EPS, AI source, PDF, SVG, PNG, JPEG) actually contains
**two different logo renderings**, not one file in two formats — mixing them
up already caused real rework once, so note the distinction carefully:

- `images/logo.png` — transparent background, the **thin outline** gear with
  a stylized "S⟩G" mark. No separate wordmark text anywhere in this file.
  Source of truth for `images/logo-full.png` (header mark) and `favicon.png`
  / `images/logo-mark.png` (gear-only crops).
- `images/logo-white-bg.jpg` — despite the filename (named before its actual
  content was checked), this is **black background**, the **bold, filled**
  gear with "SSG" lettering *and* a "SPOKANE STUMP GRINDER" wordmark line
  underneath — matches Jeff's very first reference image, a visually
  distinct design from `logo.png`, not just a recolor of it. Source of truth
  for `images/logo-banner.jpg` (homepage banner, see below).

The header brand block (`.brand`, a single `<img class="mark">`) uses
`images/logo-full.png` — the thin "S⟩G" mark trimmed to its content
bounding box, no adjacent "Spokane Stump Grinder" / "Spokane, WA" text
(removed once the real logo made it redundant). Same on all nine core pages
plus every blog post.

`favicon.png` and `images/logo-mark.png` are square gear-only crops from
`images/logo.png`, found via alpha-channel + grayscale pixel analysis (not
eyeballed coordinates) — the gear and the "S" overlap by design in that
source art, so a sliver of red bleeding into the crop's bottom-right is
expected. `images/logo-mark.png` isn't used in the header itself anymore but
is kept in case a small square icon is useful later (e.g. a social profile
picture).

`images/logo-banner.jpg` is the full bold-lettering-plus-wordmark mark
(gear + "SSG" + "SPOKANE STUMP GRINDER") cropped from `images/logo-white-bg.jpg`
to its content bounds. Used once, on `index.html` only, as a full-bleed
banner between the hero and the "What We Do" section — `<div
class="home-banner"><img></div>`, placed outside any `.wrap` so it isn't
constrained to the 1080px content width. `.home-banner img` is
`width:100%;height:auto;max-height:420px;object-fit:cover` — full aspect
ratio on mobile (natural height stays under the cap), height-capped with a
center-crop on wide desktop screens so it doesn't stretch into an
impractically tall band. Source resolution (1165×751) means it's upscaled
slightly on very wide viewports — a known tradeoff of this asset, not a bug.
If the logo is ever regenerated, redo all these crops the same
pixel-analysis way rather than guessing bounds, and double-check which of
the two source files you're actually cropping from.

The design-source formats that came with the package (`*.eps`, `*.ai`, the
whole `images/logo/` delivery folder) are gitignored — multi-megabyte files,
never linked from HTML, useful to Jeff locally (e.g. for print) but no
reason to bloat the repo. `images/Cover-size.jpg`, `images/Profile-size.jpg`,
`images/Square-size.jpg` are social-profile-formatted exports from the same
delivery, also untracked — they're for social media platforms, not the site.

`robots.txt` and `sitemap.xml` exist at the project root and are linked
(favicon) or referenced (sitemap) appropriately. `sitemap.xml` hardcodes
`https://spokanestumpgrinder.com/`; update it if the domain changes, and
remember to add/remove `<url>` entries
