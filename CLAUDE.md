...
### Logo — header mark + favicon

The real logo arrived as a full export package (EPS, AI source, PDF, SVG,
PNG, JPEG). `images/logo.png` (the transparent PNG) is the source of truth
all the derived crops below come from; `images/logo-white-bg.jpg` (flat
white background) is kept for reference/print use but not currently linked
from any page. Note the logo art itself has no separate "Spokane Stump
Grinder" wordmark line — it's just the gear + "S⟩G" mark, full stop.

The header brand block is the logo image alone now (`images/logo-full.png`,
the full mark trimmed to its content bounding box) — no adjacent "Spokane
Stump Grinder" / "Spokane, WA" text; that was removed once the real logo
made it redundant. `.brand` is a single `<img class="mark">` in the markup
on all nine core pages plus every blog post.

`favicon.png` is a square crop of just the gear element from
`images/logo.png`, generated by finding the gear's true center/radius via
alpha-channel + grayscale pixel analysis (not eyeballed coordinates) — the
gear and the wordmark's "S" overlap by design in the source art, so a small
amount of red "S" bleeding into the bottom-right of the circular crop is
expected, not a mistake. `images/logo-mark.png` is the same gear-only crop
at header-mark size — no longer used in the header itself, but kept around
in case a small square icon is useful somewhere later (e.g. a social
profile picture). If the logo is ever regenerated, redo these crops the
same pixel-analysis way rather than guessing bounds.
