...
PNG, JPEG). Only two of those are actually used on the site:
`images/logo.png` (the transparent PNG, source of truth) and
`images/logo-white-bg.jpg` (flat white background, kept for reference/print
use but not currently linked from any page).

`images/logo-mark.png` and `favicon.png` are both crops of the same gear
element from `images/logo.png`, generated by finding the gear's true
center/radius via alpha-channel + grayscale pixel analysis (not eyeballed
coordinates) — the gear and the wordmark's "S" overlap by design in the
source art, so a small amount of red "S" bleeding into the bottom-right of
the circular crop is expected, not a mistake. If the logo is ever
regenerated, redo the crop the same way rather than guessing pixel bounds.
